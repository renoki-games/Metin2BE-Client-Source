...
#ifdef HAVE_PSI_TRANSACTION_INTERFACE
static inline struct PSI_transaction_locker * inline_mysql_start_transaction(PSI_transaction_locker_state * state,
	const void * xid,
...
static inline void inline_mysql_set_transaction_gtid(PSI_transaction_locker * locker,
	const void * sid,
...
static inline void inline_mysql_set_transaction_xa_state(PSI_transaction_locker * locker,
	int xa_state)
{
	if (likely(locker != NULL))
...
static inline void inline_mysql_set_transaction_trxid(PSI_transaction_locker * locker,
	const ulonglong * trxid)
{
	if (likely(locker != NULL))
...
static inline void inline_mysql_inc_transaction_savepoints(PSI_transaction_locker * locker,
	ulong count)
{
	if (likely(locker != NULL))
...
static inline void inline_mysql_inc_transaction_rollback_to_savepoint(PSI_transaction_locker * locker,
	ulong count)
{
	if (likely(locker != NULL))
...
static inline void inline_mysql_inc_transaction_release_savepoint(PSI_transaction_locker * locker,
	ulong count)
{
	if (likely(locker != NULL))
...
static inline void inline_mysql_rollback_transaction(struct PSI_transaction_locker * locker)
{
	if (likely(locker != NULL))
		PSI_TRANSACTION_CALL(end_transaction)(locker, false);
}

static inline void inline_mysql_commit_transaction(struct PSI_transaction_locker * locker)
{
	if (likely(locker != NULL))
